...
TerrainContactData apply_weather_to_terrain_contact(TerrainContactData contact,
                                                    const WeatherSample& weather) noexcept {
  if (contact.runway_override_active ||
      contact.surface_material == geo_terrain::TerrainSurfaceMaterial::kAsphalt ||
      contact.surface_material == geo_terrain::TerrainSurfaceMaterial::kConcrete ||
      contact.surface_material == geo_terrain::TerrainSurfaceMaterial::kGrass) {
    contact.weather_friction_scale = std::clamp(weather.runway_friction_scale, 0.45, 1.0);
  } else {
    contact.weather_friction_scale = 1.0;
  }
  return contact;
}
